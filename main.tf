...
resource "azurerm_virtual_network" "elk_vnet" {
  ...
  ddos_protection_plan {
    id     = azurerm_network_ddos_protection_plan.elk_rg.id
    enable = false
  }
  ...
  subnet {
    ...
    security_group = azurerm_network_security_group.public_sg
  }

  subnet {
    name           = "${azurerm_resource_group.elk_rg.name}-subnet_private"
    address_prefix = var.subnet_private_tags["address_prefix"]
    security_group = azurerm_network_security_group.private_sg]
  }
